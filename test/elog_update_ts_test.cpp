...
TEST(TestUpdateTS, testChangeResolved)
{
    ...
    // Can remove if someone figures out why the serialize fails below
    std::cerr << "Persist dir is " << ERRLOG_PERSIST_PATH << " and ID is " << id
              << "\n";
    ...
    {
        // If someone ever figures out why this occasionally happens
        // in Jenkins CI, this can be removed.
        std::cerr << "Caught exception while resolving logs: " << e.what()
                  << "\n";
    }
