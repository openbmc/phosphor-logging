...
TEST_F(TestJsonSerialization, testJsonProperties)
{
    ...
    EXPECT_DOUBLE_EQ(j["jsonVersion"].get<double>(), JSON_FORMAT_VERSION);
...
TEST_F(TestJsonSerialization, testBinarySerializationUnchanged)
{
    ...
    auto outputPath = getEntrySerializePath(outputId);
    auto output = std::make_unique<Entry>(
        bus, (std::filesystem::path(OBJ_ENTRY) / idStr).string(), outputId,
        0, Entry::Level::Informational, "", {}, {}, "", outputPath,
        manager);
...
TEST_F(TestJsonSerialization, testJsonRoundTrip)
{
    ...
    auto output = std::make_unique<Entry>(
        ...
        Entry::Level::Informational, "", {}, {}, "", getEntrySerializePath(id),
        manager);
...
TEST_F(TestJsonSerialization, testDeserializeCorruptedJson)
{
    auto id = 10;
    auto jsonPath = dir / (std::to_string(id) + ".json");
...
TEST_F(TestJsonSerialization, testDeserializeMissingField)
{
    ...
    j["id"] = id;
    j["severity"] = static_cast<int>(Entry::Level::Error);
    // Intentionally omit "timestamp" and remaining fields
