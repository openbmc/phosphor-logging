...
TEST_F(SRCTest, MessageSubstitutionTest)
{
    ...
    entry.src.hexwordADFields = {
        {6, "COMPID"}, {7, "FREQUENCY"}, {8, "DURATION"}, {9, "ERRORCODE"}};
    // Values for the SRC words pointed to above
    std::vector<std::string> adData{"COMPID=0x1", "FREQUENCY=0x4",
                                    "DURATION=30", "ERRORCODE=0x01ABCDEF"};
    // Message with placeholder numbers to be substituted with SRC word 6-9
    // values. Placeholder numbers correspond to messageArgSources's index + 1
    entry.doc.message =
        "Comp %1 failed %2 times over %3 secs with ErrorCode %4";
    entry.doc.messageArgSources = {"SRCWord6", "SRCWord7", "SRCWord8",
                                   "SRCWord9"};
