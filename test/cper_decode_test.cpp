...
// Binary decode of libcper examples/memory.cperhex (280 bytes).
// Generated with: xxd -r -p <libcper>/examples/memory.cperhex.
...
// Befriended by Decoder for access to private members.
//
// Tests exercise private members through these helpers so the generated
// TEST_F subclasses need no friendship of their own.
...
TEST_F(DecoderTest, MemoryRecordParses)
{
    ...
    // The memory.cper fixture has a null notification-type GUID and a
    // Corrected (code 2) header severity.
...
TEST_F(DecoderTest, UnstartedDecoderStopsCleanly)
{
...
TEST_F(DecoderTest, SeverityMapping)
{
    ...
    // Out-of-range codes fall back to Recoverable.
