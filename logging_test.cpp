...
int main()
{
    // TEST 1 - Basic log
    log<level::DEBUG>(
            phosphor::logging::msg("Basic phosphor logging test"));

    // TEST 2 - Log with metadata field
    const char *file_name = "phosphor_logging_test.txt";
    int number = 0xFEFE;
    log<level::DEBUG>(
            msg("phosphor logging test with attribute"),
    ...
    number = 0x1234;
    elog(FILE_NOT_FOUND_ERROR(),
         (new_entry(FILE_PATH,"/tmp/phosphor_elog_test.txt")),
         (new_entry(ERRNO,number)));

    // Now read back and verify our data made it into the journal
    rc = validate_journal(FILE_PATH.md_name, "/tmp/phosphor_elog_test.txt");
    if(rc)
        return(rc);

    rc = validate_journal(ERRNO.md_name, std::to_string(number).c_str());
    if(rc)
        return(rc);
