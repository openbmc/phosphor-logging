...
def get_elog_data(i_elog_yaml,
    ...
    for error in ifile:
        ...
        if 'level' in meta_entry:
            error_lvl[fullname] = meta_entry['level']

        # Get 0th inherited error (current support - single inheritance)
        parent = None
        if 'inherits' in meta_entry:
            parent = meta_entry['inherits'][0]
        parents[fullname] = parent

        if 'meta' in meta_entry:
            tmp_meta = []
            # grab all the meta data fields and info
            for i in meta_entry['meta']:
                ...
                if ('process' in meta_entry) and (True == meta_entry['process']):
