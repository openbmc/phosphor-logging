...
/** @fn details::msg()
 *  @brief Implementation of logging::msg()
 */
    // Default implementation
template <typename Arg>
const auto msg(Arg&& arg)
{
    constexpr const char *msg_str = "MESSAGE=%s";
    const auto msg_tuple = std::make_tuple(msg_str, std::forward<Arg>(arg));
    return msg_tuple;
}
    // std::string
template <>
const auto msg<std::string>(std::string&& arg)
{
    constexpr const char *msg_str = "MESSAGE=%s";

    char *msg_value = new char [arg.length()+1];
    std::strcpy(msg_value, arg.c_str());

    const auto msg_tuple = std::make_tuple(msg_str, msg_value);
    return msg_tuple;
}
...
constexpr auto msg(Arg&& arg)
{
    const auto msg_tuple = details::msg((std::decay_t<Arg>)
                                        std::forward<Arg>(arg));
    return msg_tuple;
}
