...
bool deserializeJson(const fs::path& path, Entry& e)
{
    try
    {
        ...
        double fileVersion = j.value("jsonVersion", 0.0);
        ...
        // When JSON_FORMAT_VERSION is bumped, add migration logic here to
        // upgrade older entries (e.g. populate new fields with defaults).
        if (fileVersion < JSON_FORMAT_VERSION)
        {}
