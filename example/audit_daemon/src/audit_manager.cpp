                     ...
                     std::vector<uint8_t> data)
{
    ...
    else
    {
        ...
        if (rep.protocol().family() == AF_INET)
        {
            ...
            auto rawIpAddr =
                ntohl(reinterpret_cast<const sockaddr_in*>(rep.data())
                          ->sin_addr.s_addr);
            auto port = ntohs(
                reinterpret_cast<const sockaddr_in*>(rep.data())->sin_port);
        ...
        else if (rep.protocol().family() == AF_INET6)
        {
            ...
            auto port = ntohs(
                reinterpret_cast<const sockaddr_in6*>(rep.data())->sin6_port);
            std::memcpy(bytes.data(),
                        reinterpret_cast<const sockaddr_in6*>(rep.data())
                            ->sin6_addr.s6_addr,
                        16);
            auto scopeId = reinterpret_cast<const sockaddr_in6*>(rep.data())
                               ->sin6_scope_id;

            auto ip6Addr = boost::asio::ip::make_address_v6(bytes, scopeId);
            _auditd->LogEvent(
                msg.str() + _config->getMessage(requestId),
                ip6Addr.to_string() + "/" + std::to_string(scopeId) +
                    " port:" + std::to_string(port),
                std::string("get_hostname()"), std::string("no_tty()"));
        }
    }
}
