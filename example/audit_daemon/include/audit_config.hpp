...
using bmcwebList = std::vector<std::tuple<uint32_t, uint32_t>>;
...
class Config
{
  public:
    ...
    ipmiList getIPMIAllowList();
    bmcwebList getBMCWEBAllowList();
    std::string getMessage(uint32_t requestId);

  private:
    ipmiList ipmiAllowList;
    bmcwebList bmcwebAllowList;
