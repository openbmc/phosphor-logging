...
Manager::~Manager()
{
    // D-Bus match callback cleanup is handled automatically by unique_ptr.
    // We do NOT call reset() here to avoid potential crashes during test
    // teardown when the mock D-Bus bus may already be invalid.
    // The D-Bus daemon will clean up the match subscription automatically
