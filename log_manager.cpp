...
void Manager::commit(uint64_t transactionId, std::string errMsg)
{
    ...
    SD_JOURNAL_FOREACH_BACKWARDS(j)
    {
        ...
        rc = sd_journal_get_data(j, transactionIdVar, (const void **)&data, &length);
        if (rc < 0)
        {
        ...
        {
            const char* metaVar = metaVarStr.c_str();
            rc = sd_journal_get_data(j, metaVar, (const void **)&data, &length);
            if (rc < 0)
            {
                // Not found, continue to next metadata variable
                logging::log<logging::level::INFO>("Failed to find metadata",
                                    logging::entry("META_FIELD=%s", metaVar));
                continue;
            }
