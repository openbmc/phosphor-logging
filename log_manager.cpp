                      ...
                      std::string&& errMsg, Entry::Level errLvl)
{
    ...
    // createEntry returns empty path if entry creation was rejected (e.g., linear
    // SEL at capacity). Callers should check commit() return value: 0 means
    // rejected, non-zero means entry was created with that ID.
...
std::string Manager::getSelPolicy()
{
    ...
    {
        lg2::error(
            "Failed to get SEL policy subtree from D-Bus: {ERROR}. "
