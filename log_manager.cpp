...
Manager::~Manager()
{
    ...
    {
        if (errorFileWatchFD != -1)
        {
            if (errDirWatcherWD != -1)
            {
                inotify_rm_watch(errorFileWatchFD, errDirWatcherWD);
            }
            close(errorFileWatchFD);
        }
    }
}
...
{
    errorFileWatchFD = inotify_init1(IN_NONBLOCK);
    if (errorFileWatchFD == -1)
    {
    ...
    errDirWatcherWD = inotify_add_watch(errorFileWatchFD, errDir.c_str(), mask);
    if (errDirWatcherWD == -1)
    {
    ...
    errorFileWatchEventSource = std::make_unique<sdeventplus::source::IO>(
        event, errorFileWatchFD, EPOLLIN,
        std::bind_front(std::mem_fn(&Manager::errorFileChanged), this));
}
...
void Manager::errorFileChanged(sdeventplus::source::IO&, int, uint32_t revents)
{
    ...
    const auto bytesRead = read(errorFileWatchFD, buf.data(), buf.size());
    if (bytesRead < 0)
    {
