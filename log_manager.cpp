...
Manager::~Manager()
{
    // Explicitly clean up the D-Bus match callback before destroying the
    // Manager. Wrap in try-catch to handle potential exceptions during
    // mock D-Bus cleanup in tests.
    try
    {
        if (selPolicyChangedCallback)
        {
            selPolicyChangedCallback.reset();
        }
    }
    catch (const std::exception&)
    {
        // Ignore exceptions during cleanup - destructor must not throw
    }
    catch (...)
    {
