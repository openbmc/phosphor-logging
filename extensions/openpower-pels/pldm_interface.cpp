...
void PLDMInterface::readEID()
{
    if (!_eid)
    {
        ...
        *_eid = atoi(eid.c_str());
        ...
        {
            _eid = std::nullopt;
            log<level::ERR>("Could not read host EID file");
            throw std::exception{};
        }
    }
}
...
{
    log<level::INFO>("PLDMInterface::doSend");
...
{
    log<level::INFO>("PLDMInterface::receive");
    if (!(revents & EPOLLIN))
    {
        return;
    }
    ...
    if (rc < 0)
    {
        if (rc == PLDM_REQUESTER_INSTANCE_ID_MISMATCH)
        {
            ...
            log<level::INFO>( // TODO: remove after bring up
                "pldm_recv returned PLDM_REQUESTER_INSTANCE_ID_MISMATCH");
            return;
        }
    ...
    if (status == ResponseStatus::success)
    {
        ...
        rc = decode_new_file_resp(response, PLDM_NEW_FILE_RESP_BYTES,
                                             &completionCode);
        if (rc < 0)
        {
