                       ...
                       phosphor::logging::Entry::Level severity)
{
    ...
    // If the severity was in the registry entry, and that matches the
    // severity in the OpenBMC event log, then use it.  In every other case,
    // convert the OpenBMC event log severity to a sensible PEL severity.
    if (entry.severity && pelSeverityMatch(severity, entry.severity.value()))
    {
        _eventSeverity = entry.severity.value();
    }
    else
    {
