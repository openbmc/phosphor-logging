        ...
        if (!mfgSevStatus)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    lg2::error("Failed finding the severity in the message \
					registry ERROR={ERROR}",
                               "ERROR", entry.name.c_str());
...
UserHeader::UserHeader(Stream& pel)
{
    ...
    {
        lg2::error("Cannot unflatten user header: {EXC}", "EXC", e.what());
        _valid = false;
    }
}
    ...
    const DataInterfaceBase& dataIface) const
{
                    ...
                    [](const auto& sev) { return !sev.system.empty(); }))
    {
        ...
        {
            lg2::error("Failed trying to look up system names on D-Bus \
				ERROR={ERROR}",
                       "ERROR", e.what());
            return std::nullopt;
        }
