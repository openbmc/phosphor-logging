...
std::optional<EntrySeverity> getEntrySeverityType(const std::string& guardType)
{
    ...
    {
        lg2::error("GUARD: Unsupported GardType [{GUARD_TYPE}] was given to \
                        to get the hardware isolation entry severity type",
                   "GUARD_TYPE", guardType.c_str());
    }
    return std::nullopt;
}
                        ...
                        const DataInterfaceBase& dataIface)
{
    ...
    for (const auto& _callout : jsonCallouts)
    {
        ...
        {
            lg2::info("GUARD: Failed entry creation exception:({EXC})", "EXC",
                      e.what());
        }
    }
}
                           ...
                           const uint32_t plid)
{
    ...
    for (const auto& _callout : jsonCallouts)
    {
        try
        {
            ...
            {
                lg2::error("pdbg_targets_init failed, skipping deconfig  \
                                record update");
                return;
            }
        ...
        {
            lg2::info("Deconfig: Failed to create records, exception:({EXC})",
                      "EXC", e.what());
        }
    }
}
