...
void Manager::setupPELFileWatch()
{
    ...
    {
        lg2::error("Failed to setup inotify watch for PEL directory");
        abort();
    }

    _pelDirWatchEventSource = std::make_unique<sdeventplus::source::IO>(
        _event, _pelDirWatchFD, EPOLLIN,
        std::bind_front(std::mem_fn(&Manager::pelFileChanged), this));
}
                             ...
                             uint32_t revents)
{
    ...
    // An event for 1 PEL uses about 48B. When many PELs are processed at
    // once, as many events as there is room for can be handled in one
    // callback. A size of 48000 will allow about 1000 events to be
    // processed, with additional callbacks being needed to process the
    // remaining ones.
    std::array<uint8_t, 48 * 1000> buf{};
                                 ...
                                 const std::string& filename)
{
    ...
    linkPelToEventLog(logId->obmcID.id, path.string());
}
                                ...
                                [[maybe_unused]] const std::string& path)
{
    ...
    // Verify PEL exists in repository
    const Repository::LogID rid{Repository::LogID::Obmc{obmcLogID}};
    if (!_repo.getPELAttributes(rid))
    {
...
void Manager::updatePELEntryProperties(uint32_t obmcLogID)
{
    ...
    if (const auto it = _pelEntries.find(entryPath); it != _pelEntries.end())
    {
        ...
        if (const auto attributes = _repo.getPELAttributes(id); attributes)
        {
            ...
            // Update ManagementSystemAck
            const auto newAckValue = attr.hmcState == TransmissionState::acked;
            it->second->PELEntryIface::managementSystemAck(newAckValue, false);

            // Update Deconfig
            it->second->PELEntryIface::deconfig(attr.deconfig, false);

            // Update Guard
            it->second->PELEntryIface::guard(attr.guard, false);
            ...
            it->second->PELEntryIface::hidden(isHidden, false);
