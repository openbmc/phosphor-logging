...
{
    _pelDirWatchFD = inotify_init1(IN_NONBLOCK);
    if (_pelDirWatchFD == -1)
    {
    ...
    _pelDirWatcherWD =
        inotify_add_watch(_pelDirWatchFD, _repo.repoPath().c_str(), mask);
    if (_pelDirWatcherWD == -1)
    {
                             ...
                             uint32_t revents)
{
    ...
    // callback. A size of 64 * 1024 will allow about 1365 events to be
...
void Manager::handlePELDelete(const std::string& filename)
{
    ...
    if (pos != std::string::npos)
    {
        ...
        {
            lg2::info("Could not find PEL ID from its filename {NAME}", "NAME",
                      filename);
        }
    }
}
                                 ...
                                 const std::string& filename)
{
    ...
    lg2::info("Updated existing PEL from file: {FILE}", "FILE", filename);
