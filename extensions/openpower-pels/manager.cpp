...
std::string Manager::getCallouts(const openpower::pels::PEL& pel) const
{
    std::string str;
    ...
    if (src)
    {
        // First extract the calluout pointer and then go through
        const auto& callouts = (*src)->callouts()->callouts();
        namespace pv = openpower::pels::pel_values;
        // Index of the first entry to be in the string
        uint8_t index = 1;
...
{
    std::string callouts = getCallouts(*pel);
