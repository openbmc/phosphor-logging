                                      ...
                                      DebugData& debugData)
{
    ...
    auto isDIMMCallout = [&dataIface, &debugData](const auto& callout) mutable {
        ...
        else
        {
            ...
            lg2::error("{DIMM_ERROR_MSG}", "DIMM_ERROR_MSG",
                       isDIMMLocCode.error());
            return false;
        }
    };
    ...
    auto listOfDIMMCallouts = src->callouts()->callouts() |
                              std::views::filter(isDIMMCallout);

    std::ranges::for_each(listOfDIMMCallouts, addAdDIMMDetails);
}
