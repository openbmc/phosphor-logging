                                     ...
                                     const std::string& debug)
{
    ...
    for (auto& callout : calloutJSON)
    {
        ...
        c.locationCode = callout["LocationCode"].get<std::string>();
        c.name = callout["Name"].get<std::string>();
        c.priority = callout["Priority"].get<std::string>();

        if (callout.contains("MRU"))
        {
            c.mru = callout["MRU"].get<std::string>();
        }
 ...
 */
std::vector<device_callouts::Callout>
    calloutI2CUsingPath(int errnoValue, const std::string& devPath,
                        const nlohmann::json& calloutJSON)
{
    auto [bus, address] = getI2CSearchKeys(devPath);

    return calloutI2C(errnoValue, bus, address, calloutJSON);
}
 ...
 */
std::vector<device_callouts::Callout>
    calloutFSI(int errnoValue, const std::string& devPath,
               const nlohmann::json& calloutJSON)
{
    auto links = getFSISearchKeys(devPath);
 ...
 */
std::vector<device_callouts::Callout>
    calloutFSII2C(int errnoValue, const std::string& devPath,
                  const nlohmann::json& calloutJSON)
{
 ...
 */
std::vector<device_callouts::Callout>
    calloutFSISPI(int errnoValue, const std::string& devPath,
                  const nlohmann::json& calloutJSON)
{
 ...
 *        errno and device path.
 *
 * @param[in] errnoValue - the errno of the failure
 * @param[in] devPath - The device path
