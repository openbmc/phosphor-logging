 ...
 */
const PELValues subsystemValues = {{0x10, "processor", "Processor Subsystem"},
                                   ...
                                   {0x4D, "io_external_workstation", "I/O External Peripheral Local Work Station"},
                                   ...
                                   {0x5C, "cec_sp_hyp_iface", "CEC Hardware: Hypervisor-Service Processor Interface"},
                                   {0x5D, "cec_service_network", "CEC Hardware: Service Network"},
                                   {0x5E, "cec_sp_hostboot_iface", "CEC Hardware: Hostboot-Service Processor Interface"},
                                   ...
                                   {0x7A, "surv_hyp_lost_sp", "Hypervisor lost communication with service processor"},
                                   {0x7B, "surv_sp_lost_hyp", "Service processor lost communication with Hypervisor"},
                                   {0x7C, "surv_sp_lost_hmc", "Service processor lost communication with HMC"},
                                   {0x7D, "surv_hmc_lost_lpar", "HMC lost communication with logical partition"},
...
const PELValues severityValues = {
    ...
    {0x21, "predictive_degraded_perf", "Predictive Error, Degraded Performance"},
    {0x22, "predictive_reboot", "Predictive Error, Correctable"},
    {0x23, "predictive_reboot_degraded", "Predictive Error, Correctable, Degraded"},
    {0x24, "predictive_redundancy_loss", "Predictive Error, Redundancy Lost"},

    {0x40, "unrecoverable", "Unrecoverable Error"},
    {0x41, "unrecoverable_degraded_perf", "Unrecoverable Error, Degraded Performance"},
    {0x44, "unrecoverable_redundancy_loss", "Unrecoverable Error, Loss of Redundancy"},
    {0x45, "unrecoverable_redundancy_loss_perf", "Unrecoverable, Loss of Redundancy + Performance"},
    {0x48, "unrecoverable_loss_of_function", "Unrecoverable Error, Loss of Function"},

    {0x50, "critical", "Critical Error, Scope of Failure unknown"},
    {0x51, "critical_system_term", "Critical Error, System Termination"},
    {0x52, "critical_imminent_failure", "Critical Error, System Failure likely or imminent"},
    {0x53, "critical_partition_term", "Critical Error, Partition(s) Termination"},
    {0x54, "critical_partition_imminent_failure", "Critical Error, Partition(s) Failure likely or imminent"},

    {0x60, "diagnostic_error", "Error detected during diagnostic test"},
    {0x61, "diagnostic_error_incorrect_results", "Diagostic error, resource w/incorrect results"},
 ...
 */
const PELValues eventTypeValues = {{0x00, "na", "Not Applicable"},
...
const PELValues actionFlagsValues = {
    {0x8000, "service_action", "Service Action Required"},  {0x4000, "hidden", "Event not customer viewable"},
    {0x2000, "report", "Report Externally"},                {0x1000, "dont_report", "Do Not Report"},
    {0x0800, "call_home", "HMC Call Home"},                 {0x0100, "termination", "Service Processor Call Home Required"}};

/**
 * The possible values for the Callout Priority field in the SRC.
 */
const PELValues calloutPriorityValues = {
    {'H', "high", "Mandatory, replace all with this type as a unit"},       {'M', "medium", "Medium Priority"},
    {'A', "medium_group_a", "Medium Priority A, replace these as a group"}, {'B', "medium_group_b", "Medium Priority B, replace these as a group"},
    {'C', "medium_group_c", "Medium Priority C, replace these as a group"}, {'L', "low", "Lowest priority replacement"}};
