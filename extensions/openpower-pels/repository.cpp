...
bool Repository::sizeWarning() const
{
    ...
    {
        archiveSize += getFileDiskSize(dirEntry);
    }
    ...
    std::cout << "Duration to calculate size of archived files (in secs): "
              << diff.count() << std::endl;

    if ((archiveSize > 0) && ((_sizes.total + archiveSize) >
                              ((_maxRepoSize * warningPercentage) / 100)))
    {
        ...
        if (rc)
        {
            log<level::ERR>("Repository::sizeWarning function:Could not delele "
                            "files in archive");
        }
    }
