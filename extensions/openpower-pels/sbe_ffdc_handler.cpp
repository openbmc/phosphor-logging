    ...
    ffdcType(FFDC_TYPE_NONE), chipType(fapi2::TARGET_TYPE_PROC_CHIP)
{
    ...
    {
        lg2::error("Conversion failure errormsg({ERR})", "ERR", err.what());
        return;
    }
...
void SbeFFDC::parse(int fd)
{
    ...
    {
        lg2::error("Received more than the limit of ({SBEMAXFFDCPACKETS}) \
                                  FFDC packets, processing only ({PKTCOUNT})",
