...
DataInterface::DataInterface(sdbusplus::bus_t& bus) : _bus(bus)
{
    ...
    else
    {
        ...
        _systemdMatch = std::make_unique<decltype(_systemdMatch)::element_type>(
                    bus, sdbusRule::type::signal() + sdbusRule::member("JobRemoved") +
                    ...
                    [this](sdbusplus::message_t& msg) {
                        ...
                        if (jobUnitName == "openpower-update-bios-attr-table.service")
                        {
...
bool DataInterface::isPHALDevTreeExist() const
{
    ...
    catch (const std::exception& e)
    {
        lg2::error("Failed to check device tree {PHAL_DEVTREE_PATH} existence, "
                   "{ERROR}", "PHAL_DEVTREE_PATH", PDBG_DTB_PATH, "ERROR", e);
    }
    return false;
}
...
void DataInterface::subscribeToSystemdSignals()
{
    try
    {
        auto method = _bus.new_method_call(service_name::systemd,
                object_path::systemd, interface::systemdMgr,
                "Subscribe");
        ...
        _bus.call(method, 60000000);
    }
    catch (const sdbusplus::exception_t& e)
    {
        lg2::error("Failed to call Subscribe for systemd signal, "
               "serviceName: {SERVICE_NAME}, objPath: {OBJ_PATH}, "
               "interface: {INTERFACE} exception: {ERROR}", "SERVICE_NAME", service_name::systemd,
               "OBJ_PATH", object_path::systemd,
               "INTERFACE", interface::systemdMgr, "ERROR", e);
    }
}
