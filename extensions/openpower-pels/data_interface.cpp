...
constexpr auto resourceDump = "/xyz/openbmc_project/dump/resource/entry";
constexpr auto systemDump = "/xyz/openbmc_project/dump/system/entry";
} // namespace object_path
...
bool DataInterface::checkDumpStatus(const std::string& plat) const
{
    ...
    std::for_each(subtree.begin(), subtree.end(), [&](const auto& object) {
        const auto& path = object.first;
        const auto& service = object.second.begin()->first;
