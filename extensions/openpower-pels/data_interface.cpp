...
uint32_t DataInterface::getProgressCode(void) const
{
    ...
    // The Interface may not have updates before host loads. So, check to see if
    // there is any progress code before processing it.
    // Also make sure that the progress code bytes are accessible
    if ((progressCodeBytes.size() == 0) || (progressCodeBytes.size() < 72))
    {
        return progressCode;
    }
