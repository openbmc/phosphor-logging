...
void DataInterface::addSRCToProgressSRC(const uint64_t& time,
                                        const std::vector<uint8_t>& src) const
{
    auto method = _bus.new_method_call(service_name::bootRawProgress,
                                       object_path::bootRawSetting,
                                       interface::bootRawProgress, "Set");

    method.append(interface::bootRawProgress, "Value", time, src.size(), src);
    _bus.call(method);
}
} // namespace pels
} // namespace openpower
