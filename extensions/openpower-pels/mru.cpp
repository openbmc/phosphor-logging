...
MRU::MRU(Stream& pel)
{
    ...
    if (_size != actualSize)
    {
        lg2::warning("MRU callout section in PEL has listed size that doesn't \
                           match actual size \
                           SUBSTRUCTURE_SIZE={SUBSTRUCTURE_SIZE}, \
                           NUM_MRUS={NUM_MRUS}, ACTUAL_SIZE={ACTUAL_SIZE}",
                     "SUBSTRUCTURE_SIZE", _size, "NUM_MRUS", _mrus.size(),
