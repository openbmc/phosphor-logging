...
std::optional<std::string> SRC::getErrorDetails(detailLevel type) const
{
    char tmpPsVal[11];
        ...
        if (entry)
        {
            ...
            if (entry->src.hexwordADFields)
            {
                ...
                for (const auto& hexwordMap : adFields)
                {
                    ...
                    tmpString = tmpPsVal;
                    jsonInsert(errorOut, hexwordMap.second, tmpString, 2);
                }
            }
    ...
    SRC::getErrorMessage(const message::Entry& regEntry) const
{
    ...
    try
    {
        if (regEntry.doc.messageArgSources)
        {
            ...
            const auto& argSources = regEntry.doc.messageArgSources;
                ...
                if (*msgPointer == '%')
                {
                    ...
                    if (isdigit(*msgPointer) && wordIndex >= 1 &&
                        static_cast<uint16_t>(wordIndex) <=
                            argSourceVals.size())
                    {
                        sprintf(tmpPsVal, "0x%X", argSourceVals[wordIndex - 1]);
                        tmpString = tmpPsVal;
    ...
    if (_callouts)
    {
        ...
        for (auto& entry : callout)
        {
            ...
            if (entry->fruIdentity())
            {
                jsonInsert(
                    printOut, "FRU Type",
                    compDescrp.at(entry->fruIdentity()->failingComponentType()),
                    3);
                ...
                {
                    jsonInsert(printOut, "Location Code", entry->locationCode(),
                               3);
                }
...
std::optional<std::string> SRC::getJSON() const
{
    ...
    auto errorDetails = getErrorDetails(detailLevel::json);
    if (errorDetails)
    {
