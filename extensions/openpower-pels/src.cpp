...
void SRC::setDumpStatus(const DataInterfaceBase& dataIface)
{
    ...
    try
    {
        std::string dumpType = "bmc/entry";
        status = dataIface.checkDumpStatus(dumpType);
        if (status)
        {
            // For bmc set bit 0 of nibble [4-7] bits of byte -1 SP
            // dump
            _hexData[0] |= (0x1 << 19);
        }

        dumpType = "resource/entry";
        status = dataIface.checkDumpStatus(dumpType);
        if (status)
        {
