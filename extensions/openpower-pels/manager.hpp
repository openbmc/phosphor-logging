...
using PropertiesVariant = sdbusplus::utility::dedup_variant<bool, std::string>;
...
class Manager : public PELInterface
{
  ...
  private:
    ...
    /**
     * @brief Update PELEntry Interface with supported properties
     *
     ...
     * @param[in] obmcLogID - The OpenBMC entry log ID
     */
    void updatePELEntry(const uint32_t obmcLogID) const;
