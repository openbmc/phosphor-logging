...
void LightPath::activate(const PEL& pel)
{
    ...
    if (sai)
    {
        ...
        {
            lg2::error("Failed to assert platform SAI LED group: {EXC}", "EXC",
                       e.what());
        }
    }
}
    ...
    for (const auto& locCode : locationCodes)
    {
        ...
        catch (const std::exception& e)
        {
            lg2::error("Could not get inventory path for "
                       "location code {LOCCODE} ({EXC}).",
                       "LOCCODE", locCode, "EXC", e.what());
    ...
    const std::vector<std::string>& inventoryPaths) const
{
    for (const auto& path : inventoryPaths)
    {
        ...
        {
            lg2::info("Could not write Functional property on {PATH} ({EXC})",
                      "PATH", path, "EXC", e.what());
        }
    }
}
    ...
    const std::vector<std::string>& inventoryPaths) const
{
    for (const auto& path : inventoryPaths)
    {
        ...
        {
            lg2::info("Could not set critical association on object path {PATH}\
                        ({EXC})",
                      "PATH", path, "EXC", e.what());
        }
    }
