...
class LightPath : public Policy
{
  ...
  private:
    ...
    /**
     * @brief Asserts LEDs by setting the Functional property on
     *        the passed in inventory paths to false.
     *
     * There is code watching for that which will actually do the
     * LED asserting.
     *
     * @param[in] std::vector<std::string> - the inventory D-Bus paths
     */
    void assertLEDs(const std::vector<std::string>& inventoryPaths) const;
