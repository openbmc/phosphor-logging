...
std::string genPELJSON(T itr)
{
    ...
    try
    {
        ...
        if (pel.valid())
        {
            ...
            listStr += "\t\"" + val + "\": {\n";
            // ASCII
            val = std::string(pel.primarySRC().value()->asciiString().c_str());
            ...
            val = std::string(subsystem.c_str());
            listStr += "\t\t\"Subsystem\": \"" + val + "\",\n";
            // commit time
         ...
         it != fs::directory_iterator(); ++it)
    {
        ...
        try
        {
            ...
            if (pel.valid())
            {
                ...
                if (hidden || !actionFlags.test(14))
                {
                    PELs.insert(std::pair<uint32_t, BCDTime>(
                        pel.id(), pel.privateHeader().commitTimestamp()));
                }
            }
        }
        catch (std::exception& e)
...
int main(int argc, char** argv)
{
    ...
    app.add_flag("-r", listPELDescOrd, "List PELS in desc order.");
    ...
    else if (listPEL && !listPELDescOrd && !listPELShowHidden)
    {

        printList(listPELDescOrd, listPELShowHidden);
    }
    else if (listPEL && listPELDescOrd)
    {
        printList(listPELDescOrd, listPELShowHidden);
    }
    else if (listPEL && listPELShowHidden)
    {
