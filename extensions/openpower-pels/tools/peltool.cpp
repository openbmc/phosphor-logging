               ...
               const std::optional<std::regex>& scrubRegex)
{
    ...
    {
        std::cout << "[\n"
                  << "    \"No PELs found meeting criteria\"\n]" << std::endl;
    }
}
 ...
 * @return std::optional<std::regex> - SRC regex object
 */
std::optional<std::regex> genRegex(std::string& scrubFile)
{
    std::string pattern;
    std::ifstream contents(scrubFile);
    if (contents.fail())
    {
        std::cerr << "SRC scrubbing disabled: can't open " << scrubFile
                  << std::endl;
        return std::nullopt;
    }
    std::string line;
    ...
    {
        const std::string errorMsg = "SRC scrubbing disabled: ";
        if (e.code() == std::regex_constants::error_collate)
            std::cerr << errorMsg << "invalid collating element request.\n";
        else if (e.code() == std::regex_constants::error_ctype)
            std::cerr << errorMsg << "invalid character class.\n";
            ...
            std::cerr << errorMsg << "invalid back reference.\n";
        else if (e.code() == std::regex_constants::error_brack)
            std::cerr << errorMsg << "mismatched bracket ([ or ]).\n";
