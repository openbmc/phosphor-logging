...
void printList(bool order, bool hidden)
{
    ...
    if (order)
    {
        for (std::map<LogID, BCDTime>::iterator i = idsToPELs.begin();
             i != idsToPELs.end(); i++)
        {
            listStr += genPELJSON<std::map<LogID, BCDTime>::iterator>(i);
        }
    }
    else
    {

        for (std::map<LogID, BCDTime>::reverse_iterator i = idsToPELs.rbegin();
             i != idsToPELs.rend(); i++)
        {

            listStr +=
                genPELJSON<std::map<LogID, BCDTime>::reverse_iterator>(i);
        }
    }
