                                    ...
                                    void* appPrivData)
{
    ...
    catch (const std::exception& e)
    {
        ...
        lg2::error("getLocationCode({LOC_CODE}): Exception({EXC})", "LOC_CODE",
                   pdbg_target_path(target), "EXC", e.what());
    }
                       ...
                       TargetInfo& targetInfo)
{
    ...
    if (ret == 0)
    {
        ...
        lg2::error("Given ATTR_PHYS_BIN_PATH value {FMT} not found in phal \
			device tree",
                   "FMT", fmt);
        return false;
    }
...
static std::string getPelPriority(const std::string& phalPriority)
{
    ...
    {
        lg2::error("Unsupported phal priority({PHAL_PRIORITY}) is given to get \
                        pel priority format","PHAL_PRIORITY", phalPriority);
        return "H";
    }
